...
def data_scrape(since):
    ...
    # Lambda Data Munging
    lambda_scrape_df = pd.DataFrame(
        lambda_scrape, columns=["datetime", "filename", "file_url", "file_data"]
    )
    ...
    lambda_data_df = lambda_data_df.rename(
        columns={3: "SCEDTimeStamp", 5: "RepeatedHourFlag", 6: "SystemLambda"}
    )
    ...
    final_lambda_df["SCEDTimeStamp"] = pd.to_datetime(
        final_lambda_df["SCEDTimeStamp"]
    ).apply(lambda t: t.replace(second=0))

    # Wind Data Munging
    wind_scrape_df = pd.DataFrame(
        wind_scrape, columns=["datetime", "filename", "file_url", "file_data"]
    )
    ...
    wind_data_df = wind_data_df.rename(
        columns={
            0: "SCEDTimeStamp",
            2: "SYSTEM_WIDE",
            3: "LZ_SOUTH_HOUSTON",
    ...
    final_wind_df["SCEDTimeStamp"] = pd.to_datetime(
        final_wind_df["SCEDTimeStamp"]
    ).apply(lambda t: t.replace(second=0))

    # Create Final Dataframe
    final_df_df = pd.merge(
        final_lambda_df, final_wind_df, on=["SCEDTimeStamp"], how="outer"
    ).sort_values("SCEDTimeStamp")
